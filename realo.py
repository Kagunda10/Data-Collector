...
def get_realo_city_links():
    ...
    for each in main_page_soup.findAll('li', attrs={'data-id':'listContainer'}):
        if each.div.string.strip() == 'Houses for sale':
            city_houses_for_sale.append(['https://www.realo.be{}'.format(x.a['href']) for x in each.ul.findAll('li')])
        elif each.div.string.strip() == 'Houses to rent':
            city_houses_for_rent.append(['https://www.realo.be{}'.format(x.a['href']) for x in each.ul.findAll('li')])
...
def get_listing_links_list():
    ...
    results_list = []
    for each in main_page_soup.findAll('li', attrs={'data-id': 'componentEstateListGridItem'}):
        if each.div.div.a:
...
def get_apartment_info():
    ...
    # price = main_page_soup.find('li', attrs={'class':'col module module-price'}).find('div', attrs={'class':'value'}).span.text.strip())
    habitable_area = main_page_soup.find('div', attrs={'class': 'component-property-features'}).div.table.tbody.findAll('tr')[3].findAll('td')[1].text
